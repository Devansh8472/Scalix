...
function LogoMark({ compact = false }: { compact?: boolean }) {
  return (
    <div className="inline-flex items-center" aria-label="Scalix Marketing Solutions">
      <div className={`mr-[1px] shrink-0 ${compact ? 'h-9 w-9' : 'h-11 w-11'}`}>
        <img src={scalixVisualmark} alt="" aria-hidden="true" className="h-full w-full object-contain" />
      </div>
      <div className={`relative overflow-hidden ${compact ? 'h-14 w-44' : 'h-16 w-52'}`}>
        <img
          src={scalixLogo}
...
function PlaceholderImage({
  ...
  return (
    ...
    >
      <div className="flex h-full items-center justify-center rounded-[1.5rem] border border-dashed text-center text-sm leading-7 text-slate-500" style={{ borderColor: premium ? `${BRAND.orange}35` : 'rgba(148,163,184,0.25)', backgroundColor: premium ? 'rgba(255,255,255,0.65)' : 'rgba(255,255,255,0.03)' }}>
        {imageSrc ? (
          <img
            src={imageSrc}
            alt={imageAlt ?? label}
      ...
      {showVisualmark ? (
        <div className="pointer-events-none absolute right-5 top-5 flex h-14 w-14 items-center justify-center rounded-2xl border border-white/40 bg-white/70 shadow-lg backdrop-blur">
          <img src={scalixVisualmark} alt="" aria-hidden="true" className="h-9 w-9 object-contain" />
        </div>
      ) : null}
    </div>
  )
...
function SectionLabel({ children }: { children: ReactNode }) {
  return (
    <div
      ...
      style={{ borderColor: `${BRAND.orange}28`, backgroundColor: `${BRAND.orange}12`, color: BRAND.orange }}
    >
      <img src={scalixVisualmark} alt="" aria-hidden="true" className="h-4 w-4 shrink-0 object-contain" />
      <span>{children}</span>
    </div>
  )
}
...
export default function ScalixDummyWebsite() {
  ...
  return (
    <div className="site-shell min-h-screen" style={{ backgroundColor: brand.soft, color: brand.dark }}>
      {/* ─── Sticky header ─── */}
      <header className={`nav-header ${scrolled ? 'nav-header--scrolled' : ''}`}>
        <div className="nav-shell mx-auto flex w-full max-w-7xl items-center justify-between">
          ...
          {/* Desktop nav links */}
          <nav aria-label="Primary" className="nav-core hidden items-center lg:flex">
            {navItems.map((item) => (
              <a
                key={item.href}
                href={item.href}
      ...
      <main>
        {/* ═══ HERO ═══ */}
        <section id="home" className="reveal reveal-delay-1 relative overflow-x-clip">
          <div
          ...
          />
          <div className={`${sectionInset} relative grid gap-12 pt-6 pb-20 lg:grid-cols-[1.1fr_0.9fr] lg:items-start lg:pt-8 lg:pb-28`}>
            <div>
              ...
              <div className="mt-8 inline-flex max-w-2xl items-center gap-3 rounded-2xl border bg-white/80 px-4 py-3 shadow-sm backdrop-blur" style={{ borderColor: 'rgba(11,74,125,0.16)' }}>
                ...
                <span className="text-sm font-semibold text-slate-700">
                  Official Scalix visual identity integrated across your premium PPC positioning.
                </span>
              </div>
              <div className="mt-10 grid max-w-2xl gap-4 sm:grid-cols-3">
                {heroFeatures.map(([title, desc, Icon]) => (
                  <div key={title} className="lift-card rounded-[1.5rem] border bg-white p-4 shadow-sm" style={{ borderColor: 'rgba(148,163,184,0.18)' }}>
            ...
            <div className="hero-card-wrapper relative z-10 lg:sticky lg:mt-16 lg:w-[125%] xl:w-[140%] max-w-none" style={{ top: 'calc(var(--nav-height) + 2rem)' }}>
              ...
              <div className="pointer-events-none absolute -right-3 -top-3 z-10 flex h-16 w-16 items-center justify-center rounded-2xl border border-white/50 bg-white/80 shadow-xl backdrop-blur-md">
                <img src={scalixVisualmark} alt="" aria-hidden="true" className="h-10 w-10 object-contain" />
              </div>
              {/* Floating accent dots */}
              <div className="pointer-events-none absolute -bottom-4 -left-4 h-20 w-20 rounded-full float-anim" style={{ background: 'radial-gradient(circle, rgba(242,140,34,0.25), transparent 70%)', animationDelay: '0.5s' }} />
              <div className="pointer-events-none absolute -right-6 top-1/3 h-16 w-16 rounded-full float-anim" style={{ background: 'radial-gradient(circle, rgba(11,74,125,0.2), transparent 70%)', animationDelay: '1.5s' }} />
            </div>
          </div>
        </section>
        ...
        {/* ═══ WHO WE ARE ═══ */}
        <section className={`${sectionInset} reveal reveal-delay-2 grid gap-12 py-20 lg:grid-cols-[0.9fr_1.1fr] lg:items-center`}>
          <div>
            <PlaceholderImage premium showVisualmark label="Founder authority section image: polished portrait or strategy consultation visual. This should eventually be replaced with your real photo for premium trust." />
          </div>
          <div>
            ...
            <p className="mt-5 max-w-2xl text-lg leading-8 text-slate-600">
              We focus on strategic clarity over flashy noise so every campaign decision supports long-term profitability and trust with serious e-commerce teams.
            </p>
            <div className="mt-8 flex flex-wrap gap-4">
              <button type="button" className="btn-press inline-flex items-center gap-2 rounded-2xl px-6 py-3 font-semibold text-white shadow-lg transition hover:scale-105" style={{ backgroundColor: brand.navy }}>
        ...
        <section className="reveal reveal-delay-3 border-y border-slate-200 bg-white">
          <div className={`${sectionInset} grid gap-12 py-20 lg:grid-cols-2 lg:items-center`}>
            <div>
              ...
              <p className="mt-6 text-lg leading-8 text-slate-600">
                We help brands move away from random decisions and into structured execution built on intent, margin logic, and scalable operating discipline.
              </p>
              <div className="mt-8 grid gap-4 sm:grid-cols-2">
                {missionItems.map(([item, Icon]) => (
                  <div key={item} className="lift-card flex items-center gap-3 rounded-2xl border bg-slate-50 px-4 py-4 text-sm font-medium text-slate-700" style={{ borderColor: 'rgba(148,163,184,0.16)' }}>
                    <span className="icon-wrap icon-check"><Icon /></span>
                    {item}
                  </div>
                ))}
              </div>
        ...
        {/* ═══ SERVICES ═══ */}
        <section id="services" className={`${sectionInset} reveal reveal-delay-1 py-20`}>
          ...
          <div className="grid gap-8 lg:grid-cols-3">
            {services.map((service) => (
              <div key={service.title} className="lift-card group rounded-[2rem] border bg-white p-6 shadow-xl shadow-slate-200/60" style={{ borderColor: 'rgba(148,163,184,0.18)' }}>
                <div className="mb-5">
                  <PlaceholderImage premium label={`Premium service card visual: ${service.title}`} />
                </div>
                <div className="mb-4 icon-wrap icon-bubble">
                  <service.icon />
                </div>
                <div className="text-2xl font-black" style={{ color: brand.navy }}>{service.title}</div>
                <p className="mt-4 leading-8 text-slate-600">{service.desc}</p>
                <div className="mt-6 inline-flex items-center gap-1.5 text-sm font-semibold" style={{ color: brand.orange }}>
                  Framework-led execution
                  <span className="icon-wrap transition-transform group-hover:translate-x-1" style={{ width: 14, height: 14 }}><Icons.ArrowRight /></span>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* ═══ FRAMEWORK ═══ */}
        <section id="framework" className={`${sectionInset} reveal reveal-delay-2 py-20`}>
          <div
          ...
          >
            <img src={scalixVisualmark} alt="" aria-hidden="true" className="float-anim pointer-events-none absolute -right-16 -top-16 h-64 w-64 rotate-[-12deg] opacity-[0.16]" />
            <img src={scalixVisualmark} alt="" aria-hidden="true" className="float-anim pointer-events-none absolute -bottom-14 -left-14 h-40 w-40 opacity-[0.12]" style={{ animationDelay: '3s' }} />
            <div className="mb-14 max-w-3xl">
              ...
              <p className="mt-5 text-lg leading-8 text-slate-200">
                The framework turns ad activity into a repeatable operating system, from discovery to high-intent profitability.
              </p>
            </div>
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
              {framework.map((item, index) => (
                <div
                  ...
                  style={{
                    borderColor: index % 2 === 0 ? 'rgba(242,140,34,0.28)' : 'rgba(255,255,255,0.14)',
                    background:
                      index % 2 === 0
                ...
                >
                  ...
                  <div className="text-xs font-semibold uppercase tracking-[0.28em]" style={{ color: brand.orange }}>Layer 0{index + 1}</div>
                  <div className="mt-3 text-2xl font-black text-white">{item.title}</div>
        ...
        {/* ═══ CASE STUDIES ═══ */}
        <section id="case-studies" className={`${sectionInset} reveal reveal-delay-3 py-20`}>
          <div className="mb-14 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-5 text-lg leading-8 text-slate-600">
                Each case study is positioned as an operating shift, showing how better campaign architecture can improve efficiency and control.
              </p>
            </div>
            <button type="button" className="btn-press inline-flex items-center gap-2 rounded-2xl border px-5 py-3 text-sm font-semibold transition hover:bg-white" style={{ borderColor: `${brand.navy}33`, color: brand.navy }}>
          ...
          <div className="grid gap-8 lg:grid-cols-3">
            {caseStudies.map((item) => (
              <div key={item.title} className="lift-card group rounded-[2rem] border bg-white p-6 shadow-xl shadow-slate-200/60" style={{ borderColor: 'rgba(148,163,184,0.18)' }}>
                <div className="mb-5">
                  <PlaceholderImage premium label={`Premium case study placeholder: ${item.title}`} />
                </div>
                <div className="mb-3 icon-wrap icon-bubble" style={{ background: 'linear-gradient(135deg, rgba(11,74,125,0.08), rgba(45,182,217,0.06))', borderColor: 'rgba(11,74,125,0.12)', color: brand.navy }}>
                  <Icons.BarChart />
                </div>
                <div className="text-sm font-semibold uppercase tracking-[0.22em]" style={{ color: brand.orange }}>{item.stat}</div>
                <div className="mt-3 text-2xl font-black" style={{ color: brand.navy }}>{item.title}</div>
        ...
        <section className="reveal reveal-delay-1 border-y border-slate-200 bg-white">
          <div className={`${sectionInset} grid gap-12 py-20 lg:grid-cols-[1fr_0.9fr] lg:items-center`}>
            <div>
              <SectionLabel>Founder Authority</SectionLabel>
              <h2 className="text-4xl font-black leading-tight md:text-5xl" style={{ color: brand.navy }}>
                Premium websites sell trust fast. Founder-led brands do this better.
              </h2>
              <p className="mt-6 text-lg leading-8 text-slate-600">
                Add your real photo, credentials, Amazon Ads certifications, and a short founder note here. This single section can make your site feel dramatically more credible than a standard agency template.
              </p>
              <div className="mt-8 grid gap-4 sm:grid-cols-2">
                {founderFeatures.map(([item, Icon]) => (
                  <div key={item} className="lift-card flex items-center gap-3 rounded-2xl border px-4 py-4 text-sm font-medium text-slate-700" style={{ borderColor: 'rgba(148,163,184,0.16)', backgroundColor: 'rgba(248,250,252,0.9)' }}>
                    <span className="icon-wrap icon-feature"><Icon /></span>
                    {item}
                  </div>
                ))}
              </div>
            </div>
            <PlaceholderImage premium tall showVisualmark label="Founder section placeholder: premium portrait or desk shot with laptop, dashboards, and structured brand styling. Replace with your real photo in final build." />
          </div>
        </section>
        ...
        {/* ═══ BLOGS ═══ */}
        <section id="blogs" className={`${sectionInset} reveal reveal-delay-2 py-20`}>
          <div className="mb-14 max-w-3xl">
            ...
            <p className="mt-5 text-lg leading-8 text-slate-600">
              These topics are positioned for Amazon PPC intent, service discovery, and long-term search visibility.
            </p>
          </div>
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            {blogs.map((blog, i) => (
              <div key={blog} className="lift-card group rounded-[2rem] border bg-white p-6 shadow-lg shadow-slate-200/50" style={{ borderColor: 'rgba(148,163,184,0.18)' }}>
                <div className="mb-4 flex items-center justify-between">
                  <div className="icon-wrap icon-blog">
                    <Icons.FileText />
                  </div>
                  <span className="rounded-full border px-3 py-1 text-xs font-semibold" style={{ borderColor: `${brand.orange}25`, color: brand.orange, backgroundColor: `${brand.orange}08` }}>
                    Article {String(i + 1).padStart(2, '0')}
                  </span>
                </div>
                <div className="text-sm font-semibold uppercase tracking-[0.22em]" style={{ color: brand.orange }}>SEO Blog</div>
                <div className="mt-3 text-xl font-black leading-8" style={{ color: brand.navy }}>{blog}</div>
                <p className="mt-4 text-sm leading-7 text-slate-600">
                  Built to support backend SEO, internal linking, trust, and premium topical authority in the Amazon advertising niche.
                </p>
                <div className="mt-5 inline-flex items-center gap-1.5 text-sm font-semibold transition-colors" style={{ color: brand.navy }}>
                  Read more
                  <span className="icon-wrap transition-transform group-hover:translate-x-1" style={{ width: 14, height: 14 }}><Icons.ArrowRight /></span>
                </div>
              </div>
        ...
        {/* ═══ CONTACT ═══ */}
        <section id="contact" className={`${sectionInset} reveal reveal-delay-3 pb-24`}>
          <div
            className="relative overflow-hidden rounded-[2.25rem] border p-10 shadow-2xl md:p-14"
            style={{
              borderColor: `${brand.orange}22`,
          ...
          >
            <img src={scalixVisualmark} alt="" aria-hidden="true" className="float-anim pointer-events-none absolute -right-20 -bottom-16 h-72 w-72 opacity-[0.16]" />
            <div className="max-w-3xl">
              <SectionLabel>Contact</SectionLabel>
              <h2 className="text-4xl font-black leading-tight text-white md:text-6xl">
                Ready to turn your Amazon PPC into a cleaner, more profitable system?
              </h2>
              <p className="mt-6 text-lg leading-8 text-slate-200">
                Use this as your premium conversion block to move qualified visitors into strategic conversations quickly.
              </p>
              <div className="mt-8 flex flex-wrap gap-4">
                <button type="button" className="btn-press inline-flex items-center gap-2 rounded-2xl px-6 py-4 font-semibold text-white shadow-xl transition hover:scale-105" style={ctaPrimaryStyle}>
                  <span className="icon-wrap" style={{ width: 18, height: 18 }}><Icons.Mail /></span>
                  Get Free PPC Audit
                </button>
                <button type="button" className="btn-press inline-flex items-center gap-2 rounded-2xl border border-white/20 px-6 py-4 font-semibold text-white transition hover:bg-white/5">
                  <span className="icon-wrap" style={{ width: 18, height: 18 }}><Icons.Phone /></span>
                  Book Strategy Call
                </button>
              </div>
            </div>
      ...
      <footer className="border-t border-slate-200 bg-white">
        <div className="mx-auto grid max-w-7xl gap-10 px-6 py-12 md:grid-cols-4">
          <div>
            ...
            <p className="mt-4 text-sm leading-7 text-slate-500">
              Premium website shell aligned to your colors, founder-led positioning, and higher-ticket Amazon PPC authority.
            </p>
            <div className="mt-5 flex items-center gap-2">
              <a href="#" className="icon-wrap icon-social" aria-label="LinkedIn"><Icons.Linkedin /></a>
              <a href="#" className="icon-wrap icon-social" aria-label="Twitter"><Icons.Twitter /></a>
              <a href="#" className="icon-wrap icon-social" aria-label="Instagram"><Icons.Instagram /></a>
              <a href="#" className="icon-wrap icon-social" aria-label="Website"><Icons.Globe /></a>
            </div>
          </div>
          <div>
            <div className="font-bold" style={{ color: brand.navy }}>Pages</div>
          ...
          <div>
            <div className="font-bold" style={{ color: brand.navy }}>Premium Additions</div>
            <div className="mt-3 space-y-2 text-sm text-slate-500">
          ...
          <div>
            <div className="font-bold" style={{ color: brand.navy }}>Backend SEO Focus</div>
            <div className="mt-3 space-y-2 text-sm text-slate-500">
        ...
        <div className="mx-auto max-w-7xl px-6">
          <div className="footer-bottom">
            <p className="text-xs text-slate-400">© {new Date().getFullYear()} Scalix Marketing Solutions. All rights reserved.</p>
            <div className="flex items-center gap-3">
              <div className="inline-flex items-center gap-2 rounded-lg border bg-slate-50 px-3 py-1.5" style={{ borderColor: 'rgba(11,74,125,0.14)' }}>
                <img src={scalixVisualmark} alt="Scalix visualmark" className="h-5 w-5 object-contain" />
                <span className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-600">Official Visualmark</span>
              </div>
            </div>
          </div>
        </div>
